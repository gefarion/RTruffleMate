...
def _atRandom(ivkbl, rcvr, args, call_frame):
    assert isinstance(rcvr, Integer)
...
def _intDiv(ivkbl, rcvr, args, call_frame):
...
def _remainder(ivkbl, rcvr, args, call_frame):
    return rcvr.prim_remainder(args[0], ivkbl.get_universe())


def _and(ivkbl, rcvr, args, call_frame):
...
def _leftShift(ivkbl, rcvr, args, call_frame):
    right_obj = args[0]
    left      = rcvr
...
def _unsignedRightShift(ivkbl, rcvr, args, call_frame):
    right_obj = args[0]
    left      = rcvr
    universe  = ivkbl.get_universe()
...
def _bitXor(ivkbl, rcvr, args, call_frame):
    right    = args[0]
    left     = rcvr
    universe = ivkbl.get_universe()
...
def _to(ivkbl, rcvr, args, call_frame):
    assert isinstance(rcvr, Integer)
    arg = args[0]
...
def _max(ivkbl, rcvr, args, call_frame):
    assert isinstance(rcvr, Integer)
    arg = args[0]
    assert isinstance(arg, Integer)
