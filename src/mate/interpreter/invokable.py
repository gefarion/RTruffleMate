...
class MateInvokable(MateNode):
    ...
    def invoke(self, receiver, arguments, call_frame):
        ...
        sm_args = method.invoke_to_mate(receiver, [self._som_node.get_method(), Array.from_objects([environment, trueObject, receiver] + arguments)], call_frame)
        ...
        if new_args[1] == falseObject:
            ...
            return self._som_node.invoke_from_mate(receiver, new_args[3:], call_frame, environment)
        else:
            # Sigo en meta level
            return self._som_node.invoke_to_mate(receiver, new_args[3:], call_frame)
